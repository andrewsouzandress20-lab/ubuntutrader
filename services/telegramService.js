...
function getEnvVar(name) {
    ...
    if (typeof process !== 'undefined' && process.env) {
        ...
        if (name.startsWith('VITE_')) {
            ...
            if (process.env[noPrefix])
                return process.env[noPrefix];
        }
        else {
            const withPrefix = 'VITE_' + name;
            if (process.env[withPrefix])
                return process.env[withPrefix];
        }
    }
    // Vite (browser)
 ...
 */
export const sendTelegramSignal = async (assetSymbol, signal, strength, score) => {
    if (signal === 'NEUTRO') {
 ...
 */
export const sendTelegramAnalysis = async (message) => {
    try {
        const backendUrl = getEnvVar('VITE_BACKEND_URL') || '';
        const response = await fetch(`${backendUrl}/api/send-telegram`, {
            method: 'POST',
