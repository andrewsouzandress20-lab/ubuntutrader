...
export const analyzeMarket = async (candles, asset, correlations = [], events = [], context = "Standard", extraData) => {
    const ai = new GoogleGenAI({
        apiKey: import.meta.env.VITE_GEMINI_API_KEY
    });
    if (candles.length === 0)
        return { text: "Nenhum dado disponível para análise." };
    const currentPrice = candles[candles.length - 1].close;
    const priceChange = ((currentPrice - candles[0].close) / candles[0].close) * 100;
    const corrContext = correlations.map(c => `${c.name}: ${c.change.toFixed(2)}% (${c.correlation === 'positive' ? 'Correlação Direta' : 'Inversa'})`).join('\n');
    const prompt = `
