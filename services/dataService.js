import { DOW_30_TICKERS, HK_50_TICKERS } from '../types.js';
const PROXIES = [
...
const translateEvent = (title) => {
    for (const [eng, pt] of Object.entries(EVENT_TRANSLATIONS)) {
        if (title.includes(eng))
            return title.replace(eng, pt);
    }
    return title;
};
const fetchWithRetry = async (url, useProxy = true) => {
    if (!useProxy) {
        try {
...
const fetchFromYahoo = async (url) => {
...
const fetchYahooData = async (symbol, interval, range = '5d') => {
...
export const fetchCurrentPrice = async (asset) => {
    let yahooSymbol = '';
    if (asset.symbol === 'US30')
        yahooSymbol = '^DJI';
    else if (asset.symbol === 'HK50')
        yahooSymbol = '^HSI';
    else
...
export const fetchCorrelationData = async (assetSymbol) => {
    let targets = [];
    if (assetSymbol === 'HK50') {
        targets = [
    ...
    if (data?.quoteResponse?.result && data.quoteResponse.result.length > 0) {
        targets.forEach(target => {
            const quote = data.quoteResponse.result.find((r) => r.symbol === target.symbol);
            if (quote) {
                results.push({
...
export const fetchEconomicEvents = async () => {
    ...
    try {
        ...
        return data
            ...
            let impact;
            if (impactUpper.includes('HIGH'))
                impact = 'HIGH';
            else if (impactUpper.includes('MEDIUM'))
                impact = 'MEDIUM';
            else
                impact = 'LOW';
...
export const fetchMarketBreadth = async (assetSymbol) => {
    let tickers = DOW_30_TICKERS;
    if (assetSymbol === 'HK50')
        tickers = HK_50_TICKERS;
...
export const calculateVolumePressure = (candles) => {
    if (candles.length === 0)
...
export const detectOpeningGap = (candles, asset) => {
    if (candles.length < 2)
    ...
    const gapType = Math.abs(pct) > gapThreshold ? (pct > 0 ? 'up' : 'down') : 'none';
    let isFilled = false;
    if (gapType !== 'none') {
...
export const fetchRealData = async (asset, timeframe) => {
    let yahooSymbol = '';
    if (asset.symbol === 'US30')
        yahooSymbol = '^DJI';
    else if (asset.symbol === 'HK50')
