import { FVGType, ZoneType } from '../types.js';
export const detectSMCZones = (candles, config) => {
    const zones = [];
    ...
    for (let i = startIdx; i < candles.length - 1; i++) {
        ...
        if (cPrev1.high < cNext.low) {
            ...
            if (config.drawFilled || !zone.mitigated)
                zones.push(zone);
        ...
        if (cCurr.high > lastHigh)
            lastHigh = cCurr.high;
        if (cCurr.low < lastLow)
            lastLow = cCurr.low;
    }
    return zones;
};
